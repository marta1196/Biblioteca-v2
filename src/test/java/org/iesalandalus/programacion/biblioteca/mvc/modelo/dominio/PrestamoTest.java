...
public class PrestamoTest {
	...
	private static Libro libroEscrito;
	private static Libro audioLibro;
	...
	public static void inicializaAtributos() {
		...
		libroEscrito = new LibroEscrito("Cien años de soledad", "Gabriel García Márquez", 471);
		audioLibro = new AudioLibro("El retrato de Dorian Gray", "Oscar Wilde", 275);
		hoy = LocalDate.now();
	...
	public void constructorAlumnoValidoLibroValidoFechaPrestamoValidaCreaPrestamoCorrectamente() {
		Prestamo prestamo = new Prestamo(alumno, libroEscrito, hoy);
		...
		assertThat(LIBRO_NO_ESPERADO, prestamo.getLibro(), is(libroEscrito));
		assertThat(REFERENCIA_NO_ESPERADA, prestamo.getLibro(), not(sameInstance(libroEscrito)));
		assertThat(FECHA_PRESTAMO_NO_ESPERADA, prestamo.getFechaPrestamo(), is(hoy));
		prestamo = new Prestamo(alumno, audioLibro, hoy);
		assertThat(ALUMNO_NO_ESPERADO, prestamo.getAlumno(), is(alumno));
		assertThat(REFERENCIA_NO_ESPERADA, prestamo.getAlumno(), not(sameInstance(alumno)));
		assertThat(LIBRO_NO_ESPERADO, prestamo.getLibro(), is(audioLibro));
		assertThat(REFERENCIA_NO_ESPERADA, prestamo.getLibro(), not(sameInstance(audioLibro)));
		assertThat(FECHA_PRESTAMO_NO_ESPERADA, prestamo.getFechaPrestamo(), is(hoy));
	}
	...
	public void constructorAlumnoNoValidoLibroValidoFechaPrestamoValidaLanzaExcepcion() {
		...
		try {
			prestamo = new Prestamo(null, libroEscrito, hoy);
	...
	public void constructorAlumnoValidoLibroValidoFechaPrestamoNoValidaLanzaExcepcion() {
		Prestamo prestamo = null;
		try {
			prestamo = new Prestamo(alumno, libroEscrito, null);
		...
		try {
			prestamo = new Prestamo(alumno, libroEscrito, manana);
			fail(FECHA_PRESTAMO_INCORRECTA);
	...
	public void constructorPrestamoValidoCopiaPrestamoCorrectamente() {
		Prestamo prestamo1 = new Prestamo(alumno, libroEscrito, hoy);
		...
		assertThat(LIBRO_NO_ESPERADO, prestamo2.getLibro(), is(libroEscrito));
		assertThat(REFERENCIA_NO_ESPERADA, prestamo2.getLibro(), not(sameInstance(libroEscrito)));
		assertThat(FECHA_PRESTAMO_NO_ESPERADA, prestamo2.getFechaPrestamo(), is(hoy));
		prestamo1 = new Prestamo(alumno, audioLibro, hoy);
		prestamo2 = new Prestamo(prestamo1);
		assertThat(PRESTAMO_NO_ESPERADO, prestamo2, is(prestamo1));
		assertThat(REFERENCIA_NO_ESPERADA, prestamo1, not(sameInstance(prestamo2)));
		assertThat(ALUMNO_NO_ESPERADO, prestamo2.getAlumno(), is(alumno));
		assertThat(REFERENCIA_NO_ESPERADA, prestamo2.getAlumno(), not(sameInstance(alumno)));
		assertThat(LIBRO_NO_ESPERADO, prestamo2.getLibro(), is(audioLibro));
		assertThat(REFERENCIA_NO_ESPERADA, prestamo2.getLibro(), not(sameInstance(audioLibro)));
		assertThat(FECHA_PRESTAMO_NO_ESPERADA, prestamo2.getFechaPrestamo(), is(hoy));
	}
	...
	public void getPrestamoFicticioAlumnoValidoLibroValidoCreaPrestamoCorrectamente() {
		Prestamo prestamo = Prestamo.getPrestamoFicticio(alumno, libroEscrito);
		assertThat(ALUMNO_NO_ESPERADO, prestamo.getAlumno(), is(alumno));
		assertThat(REFERENCIA_NO_ESPERADA, prestamo.getAlumno(), not(sameInstance(alumno)));
		assertThat(LIBRO_NO_ESPERADO, prestamo.getLibro(), is(libroEscrito));
		assertThat(REFERENCIA_NO_ESPERADA, prestamo.getLibro(), not(sameInstance(libroEscrito)));
	...
	public void getPrestamoFicticioAlumnoNoValidoLibroValidoLanzaExcepcion() {
		Prestamo prestamo = null;
		try {
			prestamo = Prestamo.getPrestamoFicticio(null, libroEscrito);
			fail(ALUMNO_INCORRECTO);
		} catch (NullPointerException e) {
			assertThat(MENSAJE_NO_CORRECTO, e.getMessage(), is(ERROR_ALUMNO_NULO));
			assertThat(OBJETO_DEBERIA_SER_NULO, prestamo, is(nullValue()));
	...
	public void devolverFechaDevolucionValidaRealizaCorrectamenteDevolucion() {
		Prestamo prestamo = new Prestamo(alumno, libroEscrito, semanaPasada);
	...
	public void devolverFechaDevolucionNoValidaLanzaExcepcion() {
		...
		prestamo = new Prestamo(alumno, libroEscrito, semanaPasada);
		prestamo.devolver(hoy);
		try {
			prestamo.devolver(hoy);
			fail(FECHA_DEVOLUCION_INCORRECTA);
	...
	public void getPuntosDevuelvePuntosCorrectamente() {
		...
		prestamo = new Prestamo(alumno, libroEscrito, hoy.minusDays(21));
		prestamo.devolver(hoy);
		assertThat(PUNTOS_NO_ESPERADOS, prestamo.getPuntos(), is(0));
		prestamo = new Prestamo(alumno, libroEscrito, hoy.minusDays(1));
		prestamo.devolver(hoy);
		assertThat(PUNTOS_NO_ESPERADOS, prestamo.getPuntos(), is(10));
		prestamo = new Prestamo(alumno, audioLibro, hoy.minusDays(2));
		prestamo.devolver(hoy);
		assertThat(PUNTOS_NO_ESPERADOS, prestamo.getPuntos(), is(2));
		prestamo = new Prestamo(alumno, audioLibro, hoy.minusDays(5));
		prestamo.devolver(hoy);
		assertThat(PUNTOS_NO_ESPERADOS, prestamo.getPuntos(), is(1));
	}
	...
	public void equalsHashCodeComparaCorrectamente() {
		Prestamo prestamo1 = new Prestamo(alumno, libroEscrito, hoy);
		Prestamo prestamo2 = new Prestamo(prestamo1);
		Prestamo prestamo3 = new Prestamo(alumno, audioLibro, hoy);
		...
		prestamo3 = new Prestamo(Alumno.getAlumnoFicticio("kk@kk.es"), libroEscrito, hoy);
		assertThat(OBJETOS_DEBERIAN_SER_DIFERENTES, prestamo1, is(not(prestamo3)));
		assertThat(OBJETOS_DEBERIAN_SER_DIFERENTES, prestamo1.hashCode(), is(not(prestamo3.hashCode())));
	...
	public void toStringDevuelveLaCadenaEsperada() {
		Prestamo prestamo = new Prestamo(alumno, libroEscrito, semanaPasada);
		assertThat(CADENA_NO_ESPERADA, prestamo.toString(), is(String.format("alumno=(%s), libro=(%s), fecha de préstamo=%s, puntos=%d", alumno, libroEscrito, semanaPasada.format(Prestamo.FORMATO_FECHA), 0)));
		prestamo.devolver(hoy);
		assertThat(CADENA_NO_ESPERADA, prestamo.toString(), is(String.format("alumno=(%s), libro=(%s), fecha de préstamo=%s, fecha de devolución=%s, puntos=%d", alumno, libroEscrito, semanaPasada.format(Prestamo.FORMATO_FECHA), hoy.format(Prestamo.FORMATO_FECHA), 1)));
		prestamo = new Prestamo(alumno, audioLibro, semanaPasada);
		assertThat(CADENA_NO_ESPERADA, prestamo.toString(), is(String.format("alumno=(%s), libro=(%s), fecha de préstamo=%s, puntos=%d", alumno, audioLibro, semanaPasada.format(Prestamo.FORMATO_FECHA), 0)));
		prestamo.devolver(hoy);
		assertThat(CADENA_NO_ESPERADA, prestamo.toString(), is(String.format("alumno=(%s), libro=(%s), fecha de préstamo=%s, fecha de devolución=%s, puntos=%d", alumno, audioLibro, semanaPasada.format(Prestamo.FORMATO_FECHA), hoy.format(Prestamo.FORMATO_FECHA), 1)));
	}
